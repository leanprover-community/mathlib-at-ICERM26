...
public import Mathlib.Topology.Algebra.Module.Symmetric.Basic

/-!
# Topology on continuous symmetric maps

In this file we define `UniformSpace` and `TopologicalSpace` structures
on the space of continuous symmetric maps between topological vector spaces.
...
instance instTopologicalSpace : TopologicalSpace (E [Sym^ι]→L[𝕜] F) :=
...
lemma isClosed_range_toContinuousMultilinearMap [ContinuousSMul 𝕜 E] [T2Space F] :
    IsClosed (Set.range (toContinuousMultilinearMap : (E [Sym^ι]→L[𝕜] F) →
...
theorem uniformContinuous_eval_const [ContinuousSMul 𝕜 E] (x : ι → E) :
    UniformContinuous fun f : E [Sym^ι]→L[𝕜] F ↦ f x :=
  uniformContinuous_pi.1 uniformContinuous_coe_fun x

instance instIsUniformAddGroup : IsUniformAddGroup (E [Sym^ι]→L[𝕜] F) :=
...
instance instUniformContinuousConstSMul {M : Type*}
    ...
    UniformContinuousConstSMul M (E [Sym^ι]→L[𝕜] F) :=
...
theorem isUniformInducing_postcomp {G : Type*} [AddCommGroup G] [UniformSpace G]
    ...
    IsUniformInducing (g.compContinuousSymmetricMap : (E [Sym^ι]→L[𝕜] F) → (E [Sym^ι]→L[𝕜] G)) := by
...
theorem completeSpace (h : IsCoherentWith {s : Set (ι → E) | IsVonNBounded 𝕜 s}) :
    CompleteSpace (E [Sym^ι]→L[𝕜] F) := by
  ...
  · rw [(isUniformInducing_postcomp (SeparationQuotient.mkCLM _ _)
    ...
    · intro f
      use (SeparationQuotient.outCLM _ _).compContinuousSymmetricMap f
      ext
      simp
...
theorem isUniformEmbedding_restrictScalars :
    IsUniformEmbedding (restrictScalars 𝕜' : E [Sym^ι]→L[𝕜] F → E [Sym^ι]→L[𝕜'] F) := by
...
theorem uniformContinuous_restrictScalars :
    UniformContinuous (restrictScalars 𝕜' : E [Sym^ι]→L[𝕜] F → E [Sym^ι]→L[𝕜'] F) :=
...
lemma isEmbedding_toContinuousMultilinearMap :
    IsEmbedding (toContinuousMultilinearMap : (E [Sym^ι]→L[𝕜] F → _)) :=
...
instance instIsTopologicalAddGroup : IsTopologicalAddGroup (E [Sym^ι]→L[𝕜] F) :=
  isEmbedding_toContinuousMultilinearMap.topologicalAddGroup
    (toContinuousMultilinearMapLinear (R := ℕ))

@[continuity, fun_prop]
lemma continuous_toContinuousMultilinearMap :
    Continuous (toContinuousMultilinearMap : (E [Sym^ι]→L[𝕜] F → _)) :=
...
instance instContinuousConstSMul
    ...
    ContinuousConstSMul M (E [Sym^ι]→L[𝕜] F) :=
  isEmbedding_toContinuousMultilinearMap.continuousConstSMul id rfl

instance instContinuousSMul [ContinuousSMul 𝕜 F] : ContinuousSMul 𝕜 (E [Sym^ι]→L[𝕜] F) :=
...
/-- The inclusion of *symmetric* continuous multilinear maps into continuous multilinear maps
as a continuous linear map. -/
@[simps! -fullyApplied]
def toContinuousMultilinearMapCLM
    (R : Type*) [Semiring R] [Module R F] [ContinuousConstSMul R F] [SMulCommClass 𝕜 R F] :
    E [Sym^ι]→L[𝕜] F →L[R] ContinuousMultilinearMap 𝕜 (fun _ : ι ↦ E) F :=
...
lemma isClosedEmbedding_toContinuousMultilinearMap [T2Space F] :
    IsClosedEmbedding (toContinuousMultilinearMap :
      (E [Sym^ι]→L[𝕜] F) → ContinuousMultilinearMap 𝕜 (fun _ : ι ↦ E) F) :=
  ⟨isEmbedding_toContinuousMultilinearMap, isClosed_range_toContinuousMultilinearMap⟩

instance instContinuousEvalConst : ContinuousEvalConst (E [Sym^ι]→L[𝕜] F) (ι → E) F :=
  .of_continuous_forget continuous_toContinuousMultilinearMap

instance instT2Space [T2Space F] : T2Space (E [Sym^ι]→L[𝕜] F) :=
  .of_injective_continuous DFunLike.coe_injective continuous_coeFun

instance instT3Space [T2Space F] : T3Space (E [Sym^ι]→L[𝕜] F) :=
  inferInstance
...
theorem isEmbedding_restrictScalars :
    IsEmbedding (restrictScalars 𝕜' : E [Sym^ι]→L[𝕜] F → E [Sym^ι]→L[𝕜'] F) :=
...
theorem continuous_restrictScalars :
    Continuous (restrictScalars 𝕜' : E [Sym^ι]→L[𝕜] F → E [Sym^ι]→L[𝕜'] F) :=
  isEmbedding_restrictScalars.continuous

variable (𝕜') in
/-- `ContinuousSymmetricMap.restrictScalars` as a `ContinuousLinearMap`. -/
@[simps -fullyApplied apply]
def restrictScalarsCLM [ContinuousConstSMul 𝕜' F] :
    E [Sym^ι]→L[𝕜] F →L[𝕜'] E [Sym^ι]→L[𝕜'] F where
...
such that all of its values are symmetric maps,
lift it to a continuous linear map taking values in the space of continuous symmetric maps. -/
def liftCLM (f : G →L[𝕜] ContinuousMultilinearMap 𝕜 (fun _ : ι ↦ E) F)
    (hf : ∀ x v i j, [DecidableEq ι] → f x (v ∘ Equiv.swap i j) = f x v) :
    G →L[𝕜] (E [Sym^ι]→L[𝕜] F) where
  toFun x := ⟨f x, hf x⟩
...
lemma liftCLM_apply (f : G →L[𝕜] ContinuousMultilinearMap 𝕜 (fun _ : ι ↦ E) F)
    (hf : ∀ x v i j, [DecidableEq ι] → f x (v ∘ Equiv.swap i j) = f x v) (x : G) (v : ι → E) :
...
/-- Composition of a continuous symmetric map and a continuous linear map
...
@[simps! apply]
def compContinuousLinearMapCLM (f : E →L[𝕜] E') : (E' [Sym^ι]→L[𝕜] F) →L[𝕜] (E [Sym^ι]→L[𝕜] F) where
...
def apply (m : ι → E) : E [Sym^ι]→L[𝕜] F →L[𝕜] F where
  toFun c := c m
  map_add' _ _ := rfl
  map_smul' _ _ := rfl
...
@[simp]
lemma apply_apply {m : ι → E} {c : E [Sym^ι]→L[𝕜] F} : apply 𝕜 E F m c = c m := rfl

end ContinuousConstSMul

variable [ContinuousSMul 𝕜 E] {α : Type*} {p : α → E [Sym^ι]→L[𝕜] F}

theorem hasSum_eval {q : E [Sym^ι]→L[𝕜] F} (h : HasSum p q) (m : ι → E) :
...
/-- `ContinuousLinearMap.compContinuousSymmetricMap` as a bundled continuous bilinear map.

Given a continuous linear map `g : F →L[𝕜] G` and a continuous symmetric map `f : E [Sym^ι]→L[𝕜] F`,
it returns the continuous symmetric map `g ∘ f`.
...
-/
@[simps! apply_apply]
def compContinuousSymmetricMapCLM :
    (F →L[𝕜] G) →L[𝕜] (E [Sym^ι]→L[𝕜] F) →L[𝕜] (E [Sym^ι]→L[𝕜] G) where
  toFun g :=
    { toLinearMap := compContinuousSymmetricMapₗ _ _ _ _ g
      cont := by
        rw [ContinuousSymmetricMap.isEmbedding_toContinuousMultilinearMap.continuous_iff]
        exact (map_continuous <| compContinuousMultilinearMapL 𝕜 (fun _ : ι ↦ E) F G g).comp
          ContinuousSymmetricMap.continuous_toContinuousMultilinearMap }
  map_add' _ _ := by ext; simp
  map_smul' _ _ := by ext; simp
  cont := by
    rw [ContinuousLinearMap.isEmbedding_postcomp
      (ContinuousSymmetricMap.toContinuousMultilinearMapCLM 𝕜)
      ContinuousSymmetricMap.isEmbedding_toContinuousMultilinearMap |>.continuous_iff]
    exact map_continuous <|
      (precomp (ContinuousMultilinearMap 𝕜 (fun _ : ι ↦ E) G)
        ((ContinuousSymmetricMap.toContinuousMultilinearMapCLM 𝕜 : (E [Sym^ι]→L[𝕜] F) →L[𝕜] _))) ∘L
...
/-- `ContinuousLinearMap.compContinuousSymmetricMap` as a bundled continuous linear equiv.

Given a continuous linear equivalence `g : F ≃L[𝕜] G`,
this function returns the equivalence between continuous symmetric maps with codomain `F`
and continuous symmetric maps with codomain `G`
that acts by composing these maps with `g`.
-/
@[simps +simpRhs apply]
def continuousSymmetricMapCongrRight (g : F ≃L[𝕜] G) :
    (E [Sym^ι]→L[𝕜] F) ≃L[𝕜] (E [Sym^ι]→L[𝕜] G) where
  __ := g.continuousSymmetricMapCongrRightEquiv
  __ := ContinuousLinearMap.compContinuousSymmetricMapCLM 𝕜 E F G ι g.toContinuousLinearMap
  continuous_toFun := map_continuous <|
    ContinuousLinearMap.compContinuousSymmetricMapCLM 𝕜 E F G ι g.toContinuousLinearMap
  continuous_invFun := map_continuous <|
    ContinuousLinearMap.compContinuousSymmetricMapCLM 𝕜 E G F ι g.symm.toContinuousLinearMap

@[simp]
theorem _root_.ContinuousLinearEquiv.continuousSymmetricMapCongrRight_symm (g : F ≃L[𝕜] G) :
    (g.continuousSymmetricMapCongrRight (ι := ι) (E := E)).symm =
      g.symm.continuousSymmetricMapCongrRight :=
  rfl

/-- Given a continuous linear isomorphism between the domains,
generate a continuous linear isomorphism between the spaces of continuous symmetric maps.

This is `ContinuousSymmetricMap.compContinuousLinearMap` as an equivalence,
and is the continuous version of `SymmetricMap.domLCongr`. -/
@[simps apply]
def continuousSymmetricMapCongrLeft (f : E ≃L[𝕜] E') :
    E [Sym^ι]→L[𝕜] F ≃L[𝕜] (E' [Sym^ι]→L[𝕜] F) where
  __ := f.continuousSymmetricMapCongrLeftEquiv
  __ := ContinuousSymmetricMap.compContinuousLinearMapCLM (f.symm : E' →L[𝕜] E)
  toFun g := g.compContinuousLinearMap (f.symm : E' →L[𝕜] E)
  continuous_invFun :=
    (ContinuousSymmetricMap.compContinuousLinearMapCLM (f : E →L[𝕜] E')).cont
  continuous_toFun :=
    (ContinuousSymmetricMap.compContinuousLinearMapCLM (f.symm : E' →L[𝕜] E)).cont

/-- Continuous linear equivalences between the domains and the codomains
generate a continuous linear equivalence between the spaces of continuous symmetric maps. -/
@[simps! apply]
def continuousSymmetricMapCongr (e : E ≃L[𝕜] E') (e' : F ≃L[𝕜] G) :
