...
public import Mathlib.Topology.Algebra.Module.Symmetric.Topology
public import Mathlib.Analysis.Normed.Module.Multilinear.Basic

/-!
# Operator norm on the space of continuous symmetric maps

In this file we show that continuous symmetric maps
from a seminormed space to a (semi)normed space form a (semi)normed space.
We also prove basic facts about this norm
and define bundled versions of some operations on continuous symmetric maps.
...
/-- Applying a continuous symmetric map to a vector is continuous
...
instance ContinuousSymmetricMap.instContinuousEval {𝕜 ι E F : Type*}
    ...
    ContinuousEval (E [Sym^ι]→L[𝕜] F) (ι → E) F :=
...
/-!
### Continuity properties of symmetric maps

We relate continuity of symmetric maps to the inequality `‖f m‖ ≤ C * ∏ i, ‖m i‖`, in
both directions. Along the way, we prove useful bounds on the difference `‖f m₁ - f m₂‖`.
-/
namespace SymmetricMap

/-- If `f` is a continuous symmetric map on `E`
...
theorem norm_map_coord_zero (f : E [Sym^ι]→ₗ[𝕜] F) (hf : Continuous f)
    {m : ι → E} {i : ι} (hi : ‖m i‖ = 0) : ‖f m‖ = 0 :=
  f.1.norm_map_coord_zero hf hi

variable [Fintype ι]

/-- If an symmetric map in finitely many variables on seminormed spaces
...
theorem bound_of_shell_of_norm_map_coord_zero (f : E [Sym^ι]→ₗ[𝕜] F)
    (hf₀ : ∀ {m i}, ‖m i‖ = 0 → ‖f m‖ = 0)
...
/-- If a continuous symmetric map in finitely many variables on normed spaces
...
theorem bound_of_shell_of_continuous (f : E [Sym^ι]→ₗ[𝕜] F) (hfc : Continuous f)
...
/-- If an symmetric map in finitely many variables on a seminormed space is continuous,
then it satisfies the inequality `‖f m‖ ≤ C * ∏ i, ‖m i‖`,
for some `C` which can be chosen to be positive. -/
theorem exists_bound_of_continuous (f : E [Sym^ι]→ₗ[𝕜] F) (hf : Continuous f) :
    ∃ (C : ℝ), 0 < C ∧ (∀ m, ‖f m‖ ≤ C * ∏ i, ‖m i‖) :=
  f.1.exists_bound_of_continuous hf

/-- If an symmetric map `f` satisfies a boundedness property around `0`,
one can deduce a bound on `f m₁ - f m₂` using the multilinearity.
Here, we give a precise but hard to use version.
See `SymmetricMap.norm_image_sub_le_of_bound` for a less precise but more usable version.
The bound reads
`‖f m - f m'‖ ≤
  C * ‖m 1 - m' 1‖ * max ‖m 2‖ ‖m' 2‖ * max ‖m 3‖ ‖m' 3‖ * ... * max ‖m n‖ ‖m' n‖ + ...`,
where the other terms in the sum are the same products where `1` is replaced by any `i`. -/
theorem norm_image_sub_le_of_bound' [DecidableEq ι] (f : E [Sym^ι]→ₗ[𝕜] F) {C : ℝ} (hC : 0 ≤ C)
...
See `SymmetricMap.norm_image_sub_le_of_bound'` for a more precise but less usable version.
The bound is `‖f m - f m'‖ ≤ C * card ι * ‖m - m'‖ * (max ‖m‖ ‖m'‖) ^ (card ι - 1)`. -/
theorem norm_image_sub_le_of_bound (f : E [Sym^ι]→ₗ[𝕜] F) {C : ℝ} (hC : 0 ≤ C)
    (H : ∀ m, ‖f m‖ ≤ C * ∏ i, ‖m i‖) (m₁ m₂ : ι → E) :
    ‖f m₁ - f m₂‖ ≤ C * (Fintype.card ι) * (max ‖m₁‖ ‖m₂‖) ^ (Fintype.card ι - 1) * ‖m₁ - m₂‖ :=
  f.toMultilinearMap.norm_image_sub_le_of_bound hC H m₁ m₂

/-- If an symmetric map satisfies an inequality `‖f m‖ ≤ C * ∏ i, ‖m i‖`,
then it is continuous. -/
theorem continuous_of_bound (f : E [Sym^ι]→ₗ[𝕜] F) (C : ℝ) (H : ∀ m, ‖f m‖ ≤ C * ∏ i, ‖m i‖) :
    Continuous f :=
  f.toMultilinearMap.continuous_of_bound C H

/-- Construct a continuous symmetric map
from an symmetric map satisfying a boundedness condition. -/
def mkContinuous (f : E [Sym^ι]→ₗ[𝕜] F) (C : ℝ) (H : ∀ m, ‖f m‖ ≤ C * ∏ i, ‖m i‖) :
    E [Sym^ι]→L[𝕜] F :=
  { f with cont := f.continuous_of_bound C H }

@[simp] theorem coe_mkContinuous (f : E [Sym^ι]→ₗ[𝕜] F) (C : ℝ) (H : ∀ m, ‖f m‖ ≤ C * ∏ i, ‖m i‖) :
...
/-!
### Continuous symmetric maps

We define the norm `‖f‖` of a continuous symmetric map `f` in finitely many variables
as the smallest nonnegative number such that `‖f m‖ ≤ ‖f‖ * ∏ i, ‖m i‖` for all `m`.
We show that this defines a normed space structure on `E [Sym^ι]→L[𝕜] F`.
-/

namespace ContinuousSymmetricMap

variable [Fintype ι] {f : E [Sym^ι]→L[𝕜] F} {m : ι → E}

theorem bound (f : E [Sym^ι]→L[𝕜] F) : ∃ (C : ℝ), 0 < C ∧ (∀ m, ‖f m‖ ≤ C * ∏ i, ‖m i‖) :=
  f.toContinuousMultilinearMap.bound

/-- Continuous symmetric maps form a seminormed additive commutative group.
We override projection to `PseudoMetricSpace` to ensure that instances commute
in `with_reducible_and_instances`. -/
instance instSeminormedAddCommGroup : SeminormedAddCommGroup (E [Sym^ι]→L[𝕜] F) where
  toPseudoMetricSpace := .induced toContinuousMultilinearMap inferInstance
  __ := SeminormedAddCommGroup.induced _ _ (toMultilinearAddHom : E [Sym^ι]→L[𝕜] F →+ _)
  norm f := ‖f.toContinuousMultilinearMap‖

@[simp] theorem norm_toContinuousMultilinearMap (f : E [Sym^ι]→L[𝕜] F) : ‖f.1‖ = ‖f‖ := rfl
@[simp] theorem nnnorm_toContinuousMultilinearMap (f : E [Sym^ι]→L[𝕜] F) : ‖f.1‖₊ = ‖f‖₊ := rfl
@[simp] theorem enorm_toContinuousMultilinearMap (f : E [Sym^ι]→L[𝕜] F) : ‖f.1‖ₑ = ‖f‖ₑ := rfl

/-- The inclusion of `E [Sym^ι]→L[𝕜] F` into `ContinuousMultilinearMap 𝕜 (fun _ : ι ↦ E) F`
as a linear isometry. -/
@[simps!]
def toContinuousMultilinearMapLI :
    E [Sym^ι]→L[𝕜] F →ₗᵢ[𝕜] ContinuousMultilinearMap 𝕜 (fun _ : ι ↦ E) F where
  toLinearMap := toContinuousMultilinearMapLinear
  norm_map' _ := rfl

theorem norm_def (f : E [Sym^ι]→L[𝕜] F) :
...
theorem bounds_bddBelow {f : E [Sym^ι]→L[𝕜] F} :
    BddBelow {c | 0 ≤ c ∧ ∀ m, ‖f m‖ ≤ c * ∏ i, ‖m i‖} :=
  ContinuousMultilinearMap.bounds_bddBelow

theorem isLeast_opNorm (f : E [Sym^ι]→L[𝕜] F) :
    IsLeast {c : ℝ | 0 ≤ c ∧ ∀ m, ‖f m‖ ≤ c * ∏ i, ‖m i‖} ‖f‖ :=
  f.1.isLeast_opNorm

/-- The fundamental property of the operator norm of a continuous symmetric map:
`‖f m‖` is bounded by `‖f‖` times the product of the `‖m i‖`. -/
theorem le_opNorm (f : E [Sym^ι]→L[𝕜] F) (m : ι → E) : ‖f m‖ ≤ ‖f‖ * ∏ i, ‖m i‖ := f.1.le_opNorm m
...
theorem le_opNorm_mul_prod_of_le (f : E [Sym^ι]→L[𝕜] F) {b : ι → ℝ} (hm : ∀ i, ‖m i‖ ≤ b i) :
    ‖f m‖ ≤ ‖f‖ * ∏ i, b i :=
  f.1.le_opNorm_mul_prod_of_le hm

theorem le_opNorm_mul_pow_card_of_le (f : E [Sym^ι]→L[𝕜] F) {m b} (hm : ‖m‖ ≤ b) :
    ‖f m‖ ≤ ‖f‖ * b ^ Fintype.card ι :=
  f.1.le_opNorm_mul_pow_card_of_le hm

theorem le_opNorm_mul_pow_of_le {n} (f : E [Sym^Fin n]→L[𝕜] F) {m b} (hm : ‖m‖ ≤ b) :
    ‖f m‖ ≤ ‖f‖ * b ^ n :=
...
theorem ratio_le_opNorm (f : E [Sym^ι]→L[𝕜] F) (m : ι → E) : ‖f m‖ / ∏ i, ‖m i‖ ≤ ‖f‖ :=
  f.1.ratio_le_opNorm m

/-- The image of the unit ball under a continuous symmetric map is bounded. -/
theorem unit_le_opNorm (f : E [Sym^ι]→L[𝕜] F) (h : ‖m‖ ≤ 1) : ‖f m‖ ≤ ‖f‖ := f.1.unit_le_opNorm h

/-- If one controls the norm of every `f x`, then one controls the norm of `f`. -/
theorem opNorm_le_bound (f : E [Sym^ι]→L[𝕜] F) {M : ℝ} (hMp : 0 ≤ M)
...
theorem le_opNNNorm (f : E [Sym^ι]→L[𝕜] F) (m : ι → E) : ‖f m‖₊ ≤ ‖f‖₊ * ∏ i, ‖m i‖₊ :=
  f.1.le_opNNNorm m
...
theorem isLeast_opNNNorm (f : E [Sym^ι]→L[𝕜] F) :
    IsLeast {C : ℝ≥0 | ∀ m, ‖f m‖₊ ≤ C * ∏ i, ‖m i‖₊} ‖f‖₊ :=
  f.1.isLeast_opNNNorm

theorem opNNNorm_prod (f : E [Sym^ι]→L[𝕜] F) (g : E [Sym^ι]→L[𝕜] G) :
    ‖f.prod g‖₊ = max (‖f‖₊) (‖g‖₊) :=
  f.1.opNNNorm_prod g.1

theorem opNorm_prod (f : E [Sym^ι]→L[𝕜] F) (g : E [Sym^ι]→L[𝕜] G) : ‖f.prod g‖ = max (‖f‖) (‖g‖) :=
  f.1.opNorm_prod g.1

theorem opNNNorm_pi {ι' : Type*} [Fintype ι'] {F : ι' → Type*} [∀ i', SeminormedAddCommGroup (F i')]
    [∀ i', NormedSpace 𝕜 (F i')] (f : ∀ i', E [Sym^ι]→L[𝕜] F i') : ‖pi f‖₊ = ‖f‖₊ :=
...
/-- `ContinuousSymmetricMap.ofSubsingleton` as a linear isometry. -/
@[simps +simpRhs]
def ofSubsingletonLIE [Subsingleton ι] (i : ι) : (E →L[𝕜] F) ≃ₗᵢ[𝕜] (E [Sym^ι]→L[𝕜] F) where
...
@[simps]
def constOfIsEmptyLIE [IsEmpty ι] : F ≃ₗᵢ[𝕜] (E [Sym^ι]→L[𝕜] F) where
...
variable (𝕜 E F G) in
/-- `ContinuousSymmetricMap.prod` as a `LinearIsometryEquiv`. -/
@[simps]
def prodLIE : (E [Sym^ι]→L[𝕜] F) × (E [Sym^ι]→L[𝕜] G) ≃ₗᵢ[𝕜] (E [Sym^ι]→L[𝕜] (F × G)) where
  toFun f := f.1.prod f.2
  invFun f := ((ContinuousLinearMap.fst 𝕜 F G).compContinuousSymmetricMap f,
    (ContinuousLinearMap.snd 𝕜 F G).compContinuousSymmetricMap f)
  map_add' _ _ := rfl
  map_smul' _ _ := rfl
  norm_map' f := opNorm_prod f.1 f.2

variable (𝕜 E) in
/-- `ContinuousSymmetricMap.pi` as a `LinearIsometryEquiv`. -/
@[simps!]
def piLIE {ι' : Type*} [Fintype ι'] {F : ι' → Type*} [∀ i', SeminormedAddCommGroup (F i')]
    [∀ i', NormedSpace 𝕜 (F i')] :
    (∀ i', E [Sym^ι]→L[𝕜] F i') ≃ₗᵢ[𝕜] (E [Sym^ι]→L[𝕜] (∀ i, F i)) where
...
/-- `ContinuousSymmetricMap.restrictScalars` as a `LinearIsometry`. -/
@[simps]
def restrictScalarsLI : E [Sym^ι]→L[𝕜] F →ₗᵢ[𝕜'] E [Sym^ι]→L[𝕜'] F where
...
where the other terms in the sum are the same products where `1` is replaced by any `i`. -/
theorem norm_image_sub_le' [DecidableEq ι] (f : E [Sym^ι]→L[𝕜] F) (m₁ m₂ : ι → E) :
...
theorem norm_image_sub_le (f : E [Sym^ι]→L[𝕜] F) (m₁ m₂ : ι → E) :
...
/-- If a continuous symmetric map is constructed from an symmetric map via the constructor
`mkContinuous`, then its norm is bounded by the bound given to the constructor if it is
nonnegative. -/
theorem SymmetricMap.mkContinuous_norm_le (f : E [Sym^ι]→ₗ[𝕜] F) {C : ℝ} (hC : 0 ≤ C)
...
`mkContinuous`, then its norm is bounded by the bound given to the constructor if it is
nonnegative. -/
theorem SymmetricMap.mkContinuous_norm_le' (f : E [Sym^ι]→ₗ[𝕜] F) {C : ℝ}
...
theorem norm_compContinuousSymmetricMap_le (g : F →L[𝕜] G) (f : E [Sym^ι]→L[𝕜] F) :
    ‖g.compContinuousSymmetricMap f‖ ≤ ‖g‖ * ‖f‖ :=
  g.norm_compContinuousMultilinearMap_le f.1

/-- Flip arguments in `f : F →L[𝕜] E [Sym^ι]→L[𝕜] G` to get `Sym^ι⟮𝕜; E; F →L[𝕜] G⟯` -/
@[simps! apply_apply]
def flipSymmetric (f : F →L[𝕜] (E [Sym^ι]→L[𝕜] G)) : E [Sym^ι]→L[𝕜] (F →L[𝕜] G) where
  toContinuousMultilinearMap :=
    ((ContinuousSymmetricMap.toContinuousMultilinearMapCLM 𝕜).comp f).flipMultilinear
  map_eq_map_of_swap' v i j _ := by
    ext x
    simp [(f x).map_eq_map_of_swap]

end ContinuousLinearMap

theorem LinearIsometry.norm_compContinuousSymmetricMap (g : F →ₗᵢ[𝕜] G) (f : E [Sym^ι]→L[𝕜] F) :
    ‖g.toContinuousLinearMap.compContinuousSymmetricMap f‖ = ‖f‖ :=
...
theorem norm_compContinuousLinearMap_le (f : F [Sym^ι]→L[𝕜] G)
...
theorem continuous_compContinuousLinearMapCLM [Finite ι] :
    Continuous
      (compContinuousLinearMapCLM : (E →L[𝕜] F) → (F [Sym^ι]→L[𝕜] G) →L[𝕜] (E [Sym^ι]→L[𝕜] G)) := by
  rcases nonempty_fintype ι
  refine UniformConvergenceCLM.isUniformInducing_postcomp (.id 𝕜)
    (toContinuousMultilinearMapCLM 𝕜 : (E [Sym^ι]→L[𝕜] G) →L[𝕜] _)
    isUniformEmbedding_toContinuousMultilinearMap.isUniformInducing _ |>.isInducing
    |>.continuous_iff |>.mpr ?_
  change Continuous <|
    (toContinuousMultilinearMapCLM 𝕜 : (F [Sym^ι]→L[𝕜] G) →L[𝕜] _).precomp _ ∘
...
Recall that `compContinuousLinearMap f g` is the pullback of `f : F [Sym^ι]→L[𝕜] G`
along `g : E →L[𝕜] F`.
...
-/
def fderivCompContinuousLinearMap (f : F [Sym^ι]→L[𝕜] G) (g : E →L[𝕜] F) :
    (E →L[𝕜] F) →L[𝕜] (E [Sym^ι]→L[𝕜] G) :=
  liftCLM (f.1.fderivCompContinuousLinearMap (fun _ : ι ↦ g) ∘L .pi fun _ ↦ .id _ _) <| by
    intro dg v a b _
    sorry /- TODO: re-do/fix this proof!
...
@[simp]
lemma fderivCompContinuousLinearMap_apply (f : F [Sym^ι]→L[𝕜] G) (g dg : E →L[𝕜] F) (v : ι → E) :
...
def fderivCompContinuousLinearMapCLM (g : E →L[𝕜] F) :
    (F [Sym^ι]→L[𝕜] G) →L[𝕜] (E →L[𝕜] F) →L[𝕜] (E [Sym^ι]→L[𝕜] G) :=
...
@[simp]
lemma fderivCompContinuousLinearMapCLM_apply (f : F [Sym^ι]→L[𝕜] G) (g : E →L[𝕜] F) :
...
/-- Given a map `f : F →ₗ[𝕜] E [Sym^ι]→ₗ[𝕜] G` and an estimate
`H : ∀ x m, ‖f x m‖ ≤ C * ‖x‖ * ∏ i, ‖m i‖`, construct a continuous linear
map from `F` to `E [Sym^ι]→L[𝕜] G`.

In order to lift, e.g., a map `f : (E [Sym^ι]→ₗ[𝕜] F) →ₗ[𝕜] E' [Sym^ι]→ₗ[𝕜] G`
to a map `(E [Sym^ι]→L[𝕜] F) →L[𝕜] E' [Sym^ι]→L[𝕜] G`,
one can apply this construction to `f.comp ContinuousSymmetricMap.toSymmetricMapLinear`
which is a linear map from `E [Sym^ι]→L[𝕜] F` to `E' [Sym^ι]→ₗ[𝕜] G`. -/
def mkContinuousLinear (f : F →ₗ[𝕜] E [Sym^ι]→ₗ[𝕜] G) (C : ℝ)
    (H : ∀ x m, ‖f x m‖ ≤ C * ‖x‖ * ∏ i, ‖m i‖) : F →L[𝕜] E [Sym^ι]→L[𝕜] G :=
  LinearMap.mkContinuous
...
theorem mkContinuousLinear_norm_le_max (f : F →ₗ[𝕜] E [Sym^ι]→ₗ[𝕜] G) (C : ℝ)
    (H : ∀ x m, ‖f x m‖ ≤ C * ‖x‖ * ∏ i, ‖m i‖) : ‖mkContinuousLinear f C H‖ ≤ max C 0 :=
  LinearMap.mkContinuous_norm_le _ (le_max_right _ _) _

theorem mkContinuousLinear_norm_le (f : F →ₗ[𝕜] E [Sym^ι]→ₗ[𝕜] G) {C : ℝ} (hC : 0 ≤ C)
    (H : ∀ x m, ‖f x m‖ ≤ C * ‖x‖ * ∏ i, ‖m i‖) : ‖mkContinuousLinear f C H‖ ≤ C :=
  (mkContinuousLinear_norm_le_max f C H).trans_eq (max_eq_left hC)

variable {ι' : Type*} [Fintype ι']

/-- Given a map `f : E [Sym^ι]→ₗ[𝕜] (F [Sym^ι']→ₗ[𝕜] G)` and an estimate
`H : ∀ m m', ‖f m m'‖ ≤ C * ∏ i, ‖m i‖ * ∏ i, ‖m' i‖`, upgrade all `SymmetricMap`s in the type
to `ContinuousSymmetricMap`s. -/
def mkContinuousSymmetric (f : E [Sym^ι]→ₗ[𝕜] (F [Sym^ι']→ₗ[𝕜] G))
    (C : ℝ) (H : ∀ m₁ m₂, ‖f m₁ m₂‖ ≤ (C * ∏ i, ‖m₁ i‖) * ∏ i, ‖m₂ i‖) :
    E [Sym^ι]→L[𝕜] (F [Sym^ι']→L[𝕜] G) :=
  mkContinuous
    { toFun m := mkContinuous (f m) (C * ∏ i, ‖m i‖) <| H m
      map_update_add' m i x y := by ext1; simp
      map_update_smul' m i c x := by ext1; simp
      map_eq_map_of_swap' v i j _ := by ext1; simp }
    (max C 0) fun m ↦ by
...
@[simp]
theorem mkContinuousSymmetric_apply (f : E [Sym^ι]→ₗ[𝕜] (F [Sym^ι']→ₗ[𝕜] G)) {C : ℝ}
    (H : ∀ m₁ m₂, ‖f m₁ m₂‖ ≤ (C * ∏ i, ‖m₁ i‖) * ∏ i, ‖m₂ i‖) (m : ι → E) :
    ⇑(mkContinuousSymmetric f C H m) = f m :=
  rfl

theorem mkContinuousSymmetric_norm_le_max (f : E [Sym^ι]→ₗ[𝕜] (F [Sym^ι']→ₗ[𝕜] G)) {C : ℝ}
    (H : ∀ m₁ m₂, ‖f m₁ m₂‖ ≤ (C * ∏ i, ‖m₁ i‖) * ∏ i, ‖m₂ i‖) :
    ‖mkContinuousSymmetric f C H‖ ≤ max C 0 := by
  dsimp only [mkContinuousSymmetric]
  exact mkContinuous_norm_le _ (le_max_right _ _) _

theorem mkContinuousSymmetric_norm_le (f : E [Sym^ι]→ₗ[𝕜] (F [Sym^ι']→ₗ[𝕜] G)) {C : ℝ}
    (hC : 0 ≤ C) (H : ∀ m₁ m₂, ‖f m₁ m₂‖ ≤ (C * ∏ i, ‖m₁ i‖) * ∏ i, ‖m₂ i‖) :
    ‖mkContinuousSymmetric f C H‖ ≤ C :=
  (mkContinuousSymmetric_norm_le_max f H).trans_eq (max_eq_left hC)
...
/-- Continuous symmetric maps themselves form a normed group with respect to the operator norm. -/
instance instNormedAddCommGroup : NormedAddCommGroup (E [Sym^ι]→L[𝕜] F) :=
...
/-- If an symmetric map in finitely many variables on a normed space satisfies the inequality
...
theorem bound_of_shell (f : F [Sym^ι]→ₗ[𝕜] E) {ε : ι → ℝ} {C : ℝ} {c : ι → 𝕜}
