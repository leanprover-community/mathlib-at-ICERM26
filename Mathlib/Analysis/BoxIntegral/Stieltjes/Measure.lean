...
open ContinuousLinearMap MeasureTheory Set intervalIntegral StieltjesFunction ENNReal Finset
...
theorem HasStieltjesIntegral.of_continuous_of_Stieltjes (hf : ContinuousOn f (.uIcc a b)) :
  ...
  let δ' := NNReal.mk (δ / 2) (by positivity)
  ...
  have hstep_integral : ∫ x in a..b, step x ∂μ =
    ...
    rw [integral_finsetSum (fun J _ ↦ hindicator_int J)]
  ...
  have hbound_ae : ∀ᵐ x ∂μ, x ∈ Set.Ioc a b → ‖step x - f x‖ ≤ η := by
    ...
    rw [sum_eq_single J (fun K hK hKJ ↦ ?_) (fun h ↦ absurd hJπ h)]; swap
    ...
    apply (hδf _ _ _ (Ioc_subset_Icc_self hx) _).le
    · exact Icc_subset_of_box_le_Ioc hab (π.le_of_mem hJπ) htag
    simp only [Box.toSet₁_def, Set.mem_Ioc, abs_sub_lt_iff, Box.len] at hxJ' hmesh ⊢
    split_ands <;> linarith [htag.1, htag.2, hxJ'.1.le, hxJ'.2, hmesh J hJπ]
